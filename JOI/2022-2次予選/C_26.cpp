...
void Main(){
    ...
    rep(bit1, 1 << (h - 1)){
        rep(bit2, 1 << (w - 1)){
            ...
            vector<int> H, W; bool ok = 1;
            rep(h - 1) if(bit1 >> i & 1) H += i;
            rep(w - 1) if(bit2 >> i & 1) W += i;
            vector sum(60, vector(60, 0));
            int x = 0, y = 0;
            rep(h){
                x = 0;
                rep(j, w){
            ...
            rep(sum.size()) rep(j, sum[0].size()) if(sum[i][j] && sum[0][0] != sum[i][j]) ok = 0;
            if(ok) ans++;
        }
    } 
    cout << ans << endl;
