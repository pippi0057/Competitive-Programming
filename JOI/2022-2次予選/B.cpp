...
void Main(){
    ...
    rep(h) rep(j, w){
        if(0 < i && maze[i][j] != maze[i - 1][j]) g[w * i + j] += {w * (i - 1) + j, 1};
        if(0 < j && maze[i][j] != maze[i][j - 1]) g[w * i + j] += {w * i + j - 1, 1};
        if(i < h - 1 && maze[i][j] != maze[i + 1][j]) g[w * i + j] += {w * (i + 1) + j, 1};
        if(j < w - 1 && maze[i][j] != maze[i][j + 1]) g[w * i + j] += {w * i + j + 1, 1};
    }
    ll ans = dijkstra(g, 0)[h * w - 1];
