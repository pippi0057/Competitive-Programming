...
void Main(){
    int N, ans = 0, cur; cin >> N;
    vector<int> A(N), B = {-1};
    map<int, vector<int>> C;
    vector<pair<int, vector<int>>> data;
    for(auto& x : A) cin >> x;
    { int i = 0, j; while(true){
        cur = A[i];
        B.emplace_back(cur);
        for(j = 0; i + j < N && A[i + j] == cur; j++);
        i += j - 1;
        if(++i == N) break;
    }}
    cur = 1;
    B.emplace_back(-1);
    rep(i, 1, B.size() - 1) C[B[i]].emplace_back(i);
    for(auto itr = C.begin(); itr != C.end(); itr++) data.push_back({itr->first, itr->second});
    sort(all(data), [](const auto& a, const auto& b){
        return a.first < b.first;
    });
    for(Auto& [x, y] : data){
        for(Auto& z : y) if(0 < z && z < B.size() - 1){
            if(B[x - 1] > x && x < B[x + 1]) cur++;
            if(B[x - 1] < x && x > B[x + 1]) cur--;
        }
        chmax(ans, cur);
    }
