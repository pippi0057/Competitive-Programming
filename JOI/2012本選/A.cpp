...
void Main(){
    int ans = 0;
    string S; cin >> S;
    auto data = RLC::encode(S);
    rep(data.size()) if(data[i].first == 'J'){
        if(data.size() - 2 <= i) continue;
        if(data[i + 1].first != 'O') continue;
        if(data[i + 2].first != 'I') continue;
        if(data[i + 1].second != min({data[i].second, data[i + 1].second, data[i + 2].second})) continue;
        chmax(ans, min({data[i].second, data[i + 1].second, data[i + 2].second}));
    }
    cout << ans << endl;
}
