...
typedef struct data{ ll dist; int from, to; } data_t;
inline bool operator>(data_t a, data_t b){ return a.dist > b.dist; };
...
void Main(){
    ...
    auto dijkstra = [p, g](int s){
        vector<ll> dist(g.size(), inf);
        priority_queue<data_t, vector<data_t>, greater<data_t>> task;
        dist[s] = 0;
        task.push({dist[s], -1, s});
        while(task.size()){
            auto [d, from, now] = task.top(); task.pop();
            if(!(now - 1)){
                cout << d << endl;
                exit(0);
            }
            ...
            for(Auto& [to, cost] : g[now]){
                auto angle = rtod(abs((p[from] - p[now]).angle() - (p[to] - p[now]).angle()));
                if(from + 1 && (sgn(angle - 90.0) == -1 || sgn(270.0 - angle) == -1)) continue;
                task.push({dist[to], now, to});
            }
        }
    };
    dijkstra(0);
