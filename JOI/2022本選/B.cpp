...
#define u32 uint32_t
#define u64 uint64_t
constexpr ll inf = 1000000000000001000;
...
void Main(){
    ...
    for(auto& x : A) cin >> x;
    for(auto& x : B) cin >> x;
    auto check = [N, M, A, B](ll x) -> bool {
        ll cnt = 0;
        vector c(N, 0ll);
        rep(N){
            if((x + max(A[i], B[i]) - 1) / max(A[i], B[i]) <= M) cnt += M - (x + max(A[i], B[i]) - 1) / max(A[i], B[i]);
            else c[i] = x - max(A[i], B[i]) * M;
        }
        rep(N) if(c[i]){
            if(cnt < 0) break;
            cnt -= (c[i] + B[i] - 1) / B[i];
