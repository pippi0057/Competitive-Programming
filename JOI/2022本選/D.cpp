...
void Main(){
    ...
    cin >> N >> K >> M;
    Graph g(N * (M + 1));
    auto ind = [N](int i, int j) -> int { return i * N + j; };
    rep(M){
        int A, B; cin >> A >> B;
        A--; B--;
        if(A < B){
            rep(j, A, min({N - 1, A + K - 1, B})) g[ind(i, j)].push_back({ind(i, j + 1), 0});
            rep(j, A + K, min(N, B + 1)) g[ind(i, A + K - 1)].push_back({ind(i, j), 0});
        }else{
            for(int j = A; j > max({0, A - K + 1, B}); j--) g[ind(i, j)].push_back({ind(i, j - 1), 0});
            for(int j = A - K; j >= max(0, B); j--) g[ind(i, A - K + 1)].push_back({ind(i, j), 0});
        }
    }
    rep(M) rep(j, N){
        g[ind(M, j)].push_back({ind(i, j), 1});
        g[ind(i, j)].push_back({ind(M, j), 0});
    }
    cin >> Q;
    while(Q--){
        int S, T; cin >> S >> T;
        S--; T--;
        ll ans = dijkstra(g, ind(M, S))[ind(M, T)];
