...
void Main(){
    int n, now = -1; bool Taro = 1;
    cin >> n;
    vector a(2 * n, 2), ans(3, 0);
    rep(n){
        int x;
        cin >> x;
        a[x - 1] = 1;
    ...
    while(1){
        bool ok = 0;
        if(Taro){
            rep(2 * n) if(a[i] == 1 && i > now){ a[i] = 0, now = i, ok = 1; break; }
            Taro = 0;
        }else{
            rep(2 * n) if(a[i] == 2 && i > now){ a[i] = 0, now = i, ok = 1; break; }
            Taro = 1;
        }
        for(auto& x : a) ans[x]++;
        if(ans[1] && ans[2]) ans[1] = 0, ans[2] = 0;
        else break;
        if(!ok) now = -1;
    }
    rrep(i, 1, 3) cout << ans[i] << endl;
}
