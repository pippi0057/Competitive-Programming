...
void Main(){
    int n, m;
    cin >> n >> m;
    vector data(2 * n, vector(m, 'a'));
    vector<pii> ans(2 * n);
    rep(2 * n) ans[i] = {0, i};
    for(auto& v : data) for(auto& x : v) cin >> x;
    auto WL = [](char a, char b) -> int {
        if(a == b) return 0;
        if(a == 'G' && b == 'C') return 1;
        if(a == 'C' && b == 'P') return 1;
        if(a == 'P' && b == 'G') return 1;
    ...
    rep(m){
        rep(j, n){
            if(WL(data[ans[2 * j].second][i], data[ans[2 * j + 1].second][i]) == 1) ans[2 * j].first++;
            else if(WL(data[ans[2 * j].second][i], data[ans[2 * j + 1].second][i]) == -1) ans[2 * j + 1].first++;
        }
        sort(all(ans), compare);
    }
    for(Auto& [x, y] : ans) cout << y + 1 << endl;
}
