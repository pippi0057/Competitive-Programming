...
void Main(){
    int q; ll n = 1 << 20, cnt = 0;
    vector<pair<ll, int>> a(n, {-1, 0}), b;
    rep(n) a[i].second = i;
    cin >> q;
    while(q--){
        int t; ll x, m;
        ...
        if(t == 1){
            bool ok = 0;
            rep(20) if(cnt == (1 << i)) ok = 1;
            if(ok){
                b.resize(0);
                rep(n) if(a[i].first == -1) b += a[i];
            }
            rep(n) if(b[i].second == m){ m = i; break; }
            while(1){
                if(b[m].first == -1){ m = b[i].second; break; }
                m++; m %= n;
            }
            a[m].first = x;
            cnt++;
