...
void Main(){
    ll N, X, M, ans = 0, sum = 0;
    cin >> N >> X >> M;
    vector<ll> a = {X}, b;
    map<ll, int> cnt;
    while(true){
        ll now = (b.size() ? b.back() * b.back() : a.back() * a.back()) % M;
        if(cnt[now] == 0) a.emplace_back(now);
        if(cnt[now] == 1) b.emplace_back(now);
        if(cnt[now] == 2){
            rep(b.size()) a.pop_back();
            for(Auto& x : b) sum += x;
            break;
        }
        cnt[now]++;
    }
    if(N <= a.size()) rep(N) ans += a[i];
    else{
        for(Auto& x : a) ans += x;
        ans += ((N - a.size()) / b.size()) * sum;
        rep((N - a.size()) % b.size()) ans += b[i];
    }
    cout << ans << endl;
}
