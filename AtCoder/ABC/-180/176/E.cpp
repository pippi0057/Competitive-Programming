...
void Main(){
    int h, w, m, ans = 0, Hmax, Wmax;
    cin >> h >> w >> m;
    vector<int> H(3e5 + 10), W(3e5 + 10), X, Y;
    set<pii> points;
    while(m--){
        int a, b;
        cin >> a >> b;
    ...
    Hmax = ((H | sorted) | reversed)[0];
    Wmax = ((W | sorted) | reversed)[0];
    ans = Hmax + Wmax - 1;
    rep(3e5 + 10){
        if(H[i] == Hmax) X += i;
        if(W[i] == Wmax) Y += i;
    }
    for(Auto& x : X) for(Auto& y : Y) if(points.find({x, y}) == points.end()){ cout << ans + 1 << endl; return; }
    cout << ans << endl;
}
