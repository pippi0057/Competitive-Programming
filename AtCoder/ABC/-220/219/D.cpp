...
void Main(){
    int n, X, Y, x, y;
    cin >> n >> X >> Y >> x >> y;
    auto mx = [X](int x) -> ll { return min(X, x); };
    auto my = [Y](int y) -> ll { return min(Y, y); };
    vector dp(n, vector(X + 1, vector(Y + 1, inf)));
    dp[0][0][0] = 0;
    dp[0][mx(x)][my(y)] = 1;
    rep(n-1){
        cin >> x >> y;
        rep(j, X + 1) rep(k, Y + 1){
            ...
            chmin(dp[i+1][mx(j + x)][my(k + y)], dp[i][j][k] + 1);
        }
    }
    if(dp[n-1][X][Y] == inf) cout << -1 << endl;
    else cout << dp[n-1][X][Y] << endl;
}
