...
void Main(){
    int n, k;
    cin >> n >> k;
    vector<mint> fact(2001, 1);
    rep(i, 1, 2001) fact[i] = fact[i - 1] * i;
    auto comb = [fact](int a, int b) -> mint{ return fact[a] / (fact[b] * fact[a - b]); };
    rep(i, 1, k + 1){
        if(n - k - i < -1) cout << 0 << endl;
        else cout << (comb(n - k + 1, i) * comb(k - 1, i - 1)).val() << endl;
    }
}
