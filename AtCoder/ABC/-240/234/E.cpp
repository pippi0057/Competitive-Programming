...
void Main(){
    string X; cin >> X;
    auto ok = [X](string s) -> bool{
        if(s.size() == 1) return true;
        int r = s[0] - s[1];
        rep(s.size() - 1) if(s[i] - s[i + 1] != r) return false;
        return true;
    };
    if(X.size() > 10){
        ll ans = 1999999999999999999;
        rep(10) rep(j, 1, 19){
            ll t = 0, c = i;
            rep(k, j) t += c, c *= 10;
            if(stoll(X) <= t) chmin(ans, t);
        }
        cout << ans << endl;
    }else if(X.size() > 5){
        ...
        do{
            rep(10){
                string t = v.substr(0, i + 1);
                if(ok(t) && stoll(t) < stoll(s_ans) && stoll(X) <= stoll(t)) s_ans = t;
            }
        }while(next_permutation(all(v)));
        cout << s_ans << endl;
    }else{
        int ans = stoi(X);
        for(; ; ans++) if(ok(to_string(ans))){ cout << ans << endl; return; }
    }
}
