...
struct reversed_impl{
    ...
    friend string operator|(string a, reversed_impl){ reverse(all(a)); return a; }
...
void Main(){
    ...
    { int i = N - 1; for(Auto& x : S | reversed){
        if(x == 'R') left.emplace_back(i);
        else right.emplace_back(i);
        if(!i--) break;
    }}
    for(Auto& x : left | reversed) cout << x << " ";
