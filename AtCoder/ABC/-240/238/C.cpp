...
void Main(){
    ...
    mint ans = 0;
    vector<mint> data(19);
    rep(19) data[i] = ((9 * mow(10, i, 998244353) + 1) % 998244353) * ((9 * mow(10, i, 998244353)) % 998244353), data[i] /= 2;
    rep(to_string(N).size() - 1) ans += data[i];
    ll tmp = mow(10, to_string(N).size() - 1) - 1;
    if(N < 10) ans = N * (N + 1) / 2;
    else ans += ((N - tmp) % 998244353) * ((N - tmp + 1) % 998244353) / 2;
    cout << ans.val() << endl;
}
