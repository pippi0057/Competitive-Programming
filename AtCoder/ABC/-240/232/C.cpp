...
void Main(){
    ...
    rep(n) v[i] = i + 1;
    vector<pii> a(m), b(m);
    for(auto& [x, y] : a){ cin >> x >> y; if(x > y) swap(x, y); }
    for(auto& [x, y] : b){ cin >> x >> y; if(x > y) swap(x, y); }
    a |= sorted;
    do{
        vector c = b;
        for(auto& [x, y] : c){ x = v[x - 1], y = v[y - 1]; if(x > y) swap(x, y); }
        if(a == (c | sorted)){ cout << "Yes" << endl; return; }
