...
void Main(){
    ll a, N; cin >> a >> N;
    vector ans(1e6 + 10, inf);
    queue<ll> task;
    task.push(1);
    ans[1] = 0;
    while(task.size()){
        ll prev = task.front(); task.pop();
        ll now = ((prev % 10) * mow(10, to_string(prev).size()) + prev) / 10;
        if(10 <= prev && prev % 10 && now < 1e6 + 10 && chmin(ans[now], ans[prev] + 1)) task.push(now);
        if(prev * a < 1e6 + 10 && chmin(ans[prev * a], ans[prev] + 1)) task.push(prev * a);
    }
