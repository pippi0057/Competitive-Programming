...
void Main(){
    int N, ans = 0; cin >> N;
    vector<vector<int>> A(2 * N, vector(2 * N, 0));
    rep(2 * N){
        rep(j, i) A[i][j] = A[j][i];
        rep(j, i + 1, 2 * N) cin >> A[i][j];
    }
    vector<set<int>> dp(1 << (2 * N));
    dp[0].insert(0);
    rep(bit, 1 << (2 * N)){
        if(__builtin_popcount(bit) % 2) continue;
        rep(2 * N) rep(j, i) if(bit >> i & 1 && bit >> j & 1){
            for(Auto& x : dp[bit & ~(1 << i) & ~(1 << j)]) dp[bit].insert(x ^ A[i][j]);
        }
    }
    for(Auto& x : dp[(1 << (2 * N)) - 1]) chmax(ans, x);
    cout << ans << endl;
}
