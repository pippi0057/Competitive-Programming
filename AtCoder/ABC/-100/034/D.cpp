...
void Main(){
    int N, K; double ans = 0.0, ng = 100.0, mid;
    cin >> N >> K;
    vector<pair<double, double>> solt(N);
    for(auto& [w, p] : solt) cin >> w >> p;
    auto check = [N, K, &solt](double x) -> bool {
        double res = 0.0;
        sort(all(solt), [x](Auto& a, Auto& b){
            return a.first * (a.second - x) > b.first * (b.second - x);
        });
        rep(K) res += solt[i].first * (solt[i].second - x);
        if(res < 0) return false;
        return true;
    };
    while(ng - ans > 0.00000000000001){
        mid = (ans + ng) * 0.5;
