...
void Main(){
    int A, B, C; cin >> A >> B >> C;
    vector dp(110, vector(110, vector(110, -1.0)));
    rep(110) rep(j, 110) dp[100][i][j] = 0.0, dp[i][100][j] = 0.0, dp[i][j][100] = 0.0;
    auto dfs = [&dp](auto& self, int x, int y, int z) -> double{
        if(dp[x][y][z] != -1.0) return dp[x][y][z];
        return dp[x][y][z] = (
            (self(self, x + 1, y, z) + 1) * x +
            (self(self, x, y + 1, z) + 1) * y +
            (self(self, x, y, z + 1) + 1) * z
        ) / (x + y + z);
    }; dfs(dfs, A, B, C);
    cout << dp[A][B][C] << endl;
}
