...
void Main(){
    ...
    rep(n){
        int input;
        cin >> input;
        if(i % 2) a.push_back(input);
        else b.push_back(input);
    }
    rep(bit, 1 << a.size()){
        int temp = 0;
        rep(a.size()) if(bit >> i & 1) temp += a[i];
        sum_a.push_back(temp);
    }
    rep(bit, 1 << b.size()){
        int temp = 0;
        rep(b.size()) if(bit >> i & 1) temp += b[i];
        sum_b.push_back(temp);
    }
    sort(all(sum_a));
    for(auto x : sum_b){
        int temp = t - x;
        int ind = upper_bound(all(sum_a), temp) - sum_a.begin() - 1;
        if(ind == -1) continue;
        chmax(ans, sum_a[ind] + x);
    }
    cout << ans << endl;
}
