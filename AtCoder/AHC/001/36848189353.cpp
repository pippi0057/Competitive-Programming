...
void Main(clock_t START, int TIME){
    ...
    while(task.size()){
        ...
        if(!abs(a[i].r - area[i])) skip[i] = 1;
        ENDLOOP
    }
    while(1){
        int i = rand() % n, cmd = rand() % 8;
        if(skip[i]) continue;
        bool ok = 1;
        answer prev = ans[i];
        if(cmd == 0) ans[i].a -= 1;
        ...
        if(cmd == 2) ans[i].c += 1;
        if(cmd == 3) ans[i].d += 1;
        if(cmd == 4) ans[i].a += 1, ans[i].c += 1;
        if(cmd == 5) ans[i].b += 1, ans[i].d += 1;
        if(cmd == 6) ans[i].a -= 1, ans[i].c -= 1;
        if(cmd == 7) ans[i].b -= 1, ans[i].d -= 1;
        rep(j, n){
            if(i == j) continue;
            if(!judge(ans[i].a, ans[i].b, ans[i].c, ans[i].d, ans[j].a, ans[j].b, ans[j].c, ans[j].d)) ok = 0;
        }
