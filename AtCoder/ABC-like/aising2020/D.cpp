...
void Main(){
    ...
    string s;
    cin >> n >> s;
    vector<ll> dp(2e5 + 10), ans(n);
    dp[0] = 0;
    rep(i, 1, 2e5 + 10) dp[i] = dp[i % popcount(i)] + 1;
    for(Auto& x : s) pop += x == 49;
    ll plus = 0, minus = 0;
    rep(n) if(s[i] - 48 && pop - 1) plus = (plus + mow(2, n - i - 1, pop + 1)) % (pop + 1), minus = (minus + mow(2, n - i - 1, pop - 1)) % (pop - 1);
    rep(n){
        if(s[i] - 48) ans[i] = pop - 1 ? dp[(minus - mow(2, n - i - 1, pop - 1) + pop - 1) % (pop - 1)] + 1 : 0;
        else ans[i] = dp[(plus + mow(2, n - i - 1, pop + 1)) % (pop + 1)] + 1;
    }
