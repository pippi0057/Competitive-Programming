...
void Main(){
    ...
    rep(r) rep(j,c-1) cin >> a[i][j];
    rep(r-1) rep(j,c) cin >> b[i][j];
    Graph g(r * c);
    rep(r) rep(j,c){
        if(j > 0) g[i*c+j].push_back({i*c+j-1, a[i][j-1]});
        if(j < c - 1) g[i*c+j].push_back({i*c+j+1, a[i][j]});
        if(i < r - 1) g[i*c+j].push_back({(i+1)*c+j, b[i][j]});
        rep(k,1,i+1) g[i*c+j].push_back({(i-k)*c+j, k+1});
    }
    vector<ll> ans = dijkstra(g,0);
    cout << ans[r*c-1] << endl;
}
