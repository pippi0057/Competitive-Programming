...
void Main(){
    struct query { ll t, n, c; };
    ll h, w, c, q;
    cin >> h >> w >> c >> q;
    vector<query> data(q);
    vector<ll> ans(c);
    set<int> X, Y;
    for(auto& [t, n, color] : data) cin >> t >> n >> color;
    for(Auto& [t, n, color] : data | reversed){
        switch(t){
            case 1:
                if(X.insert(n).second){ h--; ans[color - 1] += w; }
                break;
            case 2:
                if(Y.insert(n).second){ w--; ans[color - 1] += h; }
                break;
        }
    }
