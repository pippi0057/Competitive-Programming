...
void Main(){
    vector<int> order = {10, 11, 12, 13, 14, 15, 16, 17, 18, 1, 2, 3, 4, 5, 6, 7, 8, 9};
    vector<string> s(2), ans(2);
    vector a(10, 0), b(10, 0);
    for(auto& x : s) cin >> x;
    vector dp(min(s[0].size(), s[1].size()), vector(10, vector(10, {0, false})));
    for(Auto& x : s[0]) a[x - '0']++;
    for(Auto& x : s[1]) b[x - '0']++;
    rep(min(s[0].size(), s[1].size())){
        if()
    }
}
