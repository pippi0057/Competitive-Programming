...
void Main(){
    ...
    auto sum = [&data](int x, int y) -> int {
        int res = data.count({x, y}) ? data[{x, y}] : 0;
        rep(8) res += data.count({x + dx[i], y + dy[i]}) ? data[{x + dx[i], y + dy[i]}] : 0;
        return res;
    };
    for(auto itr = data.begin(); itr != data.end(); itr++){
        auto [x, y] = itr->first;
        chmax(ans, sum(x, y));
        rep(8) chmax(ans, sum(x + dx[i], y + dy[i]));
    }
    cout << ans << endl;
}
