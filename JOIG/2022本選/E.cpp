...
void Main(){
    ...
    cin >> N >> M;
    vector<pii> A(N);
    vector<vector<int>> dp(N, vector<int>(M + 1, -999999999));
    for(auto& [x, y] : A) cin >> x >> y;
    dp[0][A[0].second] = max(0, A[0].first);
    rep(i, 1, N){
        rep(j, M + 1) if(A[i].second != j) chmax(dp[i][A[i].second], dp[i - 1][j] + A[i].first);
    }
    for(Auto& x : dp[N - 1]) chmax(ans, x);
    cout << ans << endl;
}
